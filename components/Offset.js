...
const Offset = ({ isOpen, onOpen, onClose }) => {
    ...
    useEffect(()=>{
        if (carbon === false) fetch(`/api/offset?address=${address}`).then(e=>e.json()).then(ca=>{
            setCarbon(ca);
    ...
    async function offsetToucanCarbon(valueInTons = 0){
        ...
        if (chain.name === 'Polygon'){
            ...
            try {
                ...
                let resp = helperInstance.connect(signer).autoOffsetExactInETH(
                    addresses.nct,
                    {value: amount}
                );
            ...
            alert(`Switch to Polygon Mainnet, You're on ${chain.name}`);
        }
    }
    ...
    return(
        <>
            <Modal isOpen={isOpen} onClose={onClose}>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>🌳 Climate Action [⚠️ Experimental]</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
                        {
                            carbon === false ? (
                                ...
                                <Flex direction="column">
                                    ...
                                    <StatGroup>
                                        ...
                                        <Stat>
                                            ...
                                            <StatNumber>0 TCO<sub>2</sub></StatNumber>
                                            <StatHelpText>
                                                Total on-chain offsets
                                            </StatHelpText>
                                        </Stat>
                                    </StatGroup>
